...
const STEPS = {
  ...
  search: {
    ...
    rows: [
      ...
      { label: 'Look up skills the same way', control: 'toggle', value: 'on', hint: 'A count and an instruction to search stay, so it still knows notes exist.' },
  ...
  expert: {
    tab: 'agents',
    note: 'Specialists your assistant consults while it works. Assign a model to a role and it is offered.',
    rows: [
      { label: 'Expert', control: 'select', value: 'Claude (command line)', hint: 'Detected automatically, and the default here — but any model can take the seat.' },
      { label: 'Programmer', control: 'select', value: 'Nobody' },
      { label: 'Reviewer', control: 'select', value: 'Corporate gateway', hint: 'Reads a finished change and says what is wrong with it.' },
      { label: 'Tester', control: 'select', value: 'Nobody' },
      { label: 'Librarian', control: 'select', value: 'Nobody', hint: 'Skills, internal docs and house conventions.' },
      { label: 'Prompt', control: 'button', value: 'Prompt', hint: 'What each specialist is told it is. Edit it to suit this codebase.' },
      { section: 'cost' },
      { label: 'What consultations cost is worth managing', control: 'toggle', value: 'off', hint: 'Only a Claude consultation reports a price, so a budget is shown only where one applies.' },
      { section: 'when to consult' },
      { label: 'Consulting advice', control: 'list', value: 'editable — the roster of who exists is added automatically', hint: 'The assistant consults on its own; this is its judgement about when that is worth it.' },
    ],
  },
  ...
  schedules: {
    ...
    rows: [
      ...
      { label: 'What it should know', control: 'text', value: 'filter by name or description…', hint: 'Search the skills, then tick the ones this job needs. All of them until you narrow it.' },
      { label: '  release-process', control: 'chips', value: 'included' , hint: 'A schedule names its skills rather than searching — it may not have search_docs, and nobody is watching.' },
  ...
  skills: {
    ...
    rows: [
      ...
      { label: 'Found by searching, not listing', control: 'list', value: 'search_docs, like tools', hint: 'Turn it off in Search if you would rather every summary sat in the prompt.' },
  ...
  tools: {
    ...
    rows: [
      ...
      { label: '“looked up” means hidden from the prompt', control: 'list', value: 'still callable — it finds it first', hint: 'The default. A shorter prompt is not a shorter tool list — withholding a capability is what Approvals is for.' },
    ],
  },
