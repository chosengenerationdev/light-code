...
- A new icon: a bulb with light coming off it, and a cross inside.

  From a hand-drawn sketch. The brand tile is unchanged — the same indigo-to-violet square and white
  mark — so it reads as the same product.

  **The cross is the bulb's filament rather than a badge on it.** Drawn as a bold filled cross it
  reads as a first-aid symbol and takes the whole icon over; drawn as the wire inside the glass it
  is what a filament actually looks like, and it is noticed rather than announced.

  **The activity-bar icon now fills its box.** The old mark stood about half the height of its
  neighbours in the activity bar, which is not a style choice — it is the icon looking like it
  matters less. It has no rays: at 24 pixels seven of them around a bulb is grey fuzz, so they stay
  on the marketplace icon where there is room for them.

  The art is generated by `scripts/generate-icons.mjs` rather than checked in as binaries alone, so
  the next change to it is a diff somebody can read.
